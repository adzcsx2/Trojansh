...
systemctl restart nginx
	...
	~/.acme.sh/acme.sh --register-account -m hoyn@$your_domain --server letsencrypt --force
 	~/.acme.sh/acme.sh --set-default-ca --server letsencrypt --force
	~/.acme.sh/acme.sh --issue -d $your_domain --nginx --server letsencrypt --force
	~/.acme.sh/acme.sh --installcert -d $your_domain --server letsencrypt --key-file /usr/local/etc/trojan/private.key --fullchain-file /usr/local/etc/trojan/cert.crt --force
	~/.acme.sh/acme.sh --upgrade --auto-upgrade --server letsencrypt --force
